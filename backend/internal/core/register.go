...
func NewRegisterUseCase(user i.IUser, token i.IToken, hash i.IHash, sessionExp int) *RegisterUseCase {
...
func (uc *RegisterUseCase) registerByEmail(ctx context.Context, input RegisterInput) (*entities.User, error) {
	...
	identity, err := entities.NewIdentity("email", user.Email, "sso.semgateam.ru")
	if err != nil {
		log.Info("invalid identity", zap.Error(err))
		return nil, err
	}

	credential, err := entities.NewCredential("password", hashed)
	if err != nil {
